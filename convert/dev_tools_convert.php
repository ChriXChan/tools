<?php
/**----------------------------------------------------+
 * 解析XML文件生成器  用于dev_tools
 +-----------------------------------------------------*/
//执行方法：php convert.php 数据类型
//例如：php convert.php map
error_reporting(E_ALL ^E_NOTICE);

$contens = file("../../dev_tools/config/config.ini");
foreach($contens as $v){
    $tmp = explode('=',$v);
    define($tmp[0], trim($tmp[1]));
}

define('ROOT',          str_replace('\\', '/', realpath(dirname(__FILE__))));
define('DATA_DIR',      DIR_ROOT."/doc/data/");
define('SRV_DATA_DIR',	DIR_ROOT.'server/src/data/');
define('TPL_DIR',       ROOT.'/tpl/');
define('TARGET_DIR',    ROOT.'/target/');
define('CLI_DATA_DIR',  DIR_ROOT.'resources/assets/config/');
...
function handleLanguage($input_file) {
    ...
    echo ">> Start convert {".$input_file."} files\n";
    ...
    echo color_green(">> convert {".$input_file."} files completed.\n");
}

function main($input_file) {
    $data_file = DIR_ROOT. "/doc/data/".$input_file.".xml";    
    $tpl_file = DIR_ROOT."/tools/convert/tpl/".$input_file.".tpl.php";    
    $target_file = DIR_ROOT."/server/src/data/".$input_file.".erl";    
    $cli_tpl_file = DIR_ROOT."/tools/convert/tpl/".$input_file.".clitpl.php";
    $cli_target_file = DIR_ROOT."/resources/assets/config/".$input_file.".json";
    ...
    if (file_exists(realpath($cli_tpl_file))) {
        ...
        // 打包客户端语言包文件
        echo ">> Please wait,client make language date ...\n";
        $lang_str = mk_cli_lang(CLI_DATA_DIR . "language/");
        file_put_contents(CLI_DATA_DIR.'language.json', $lang_str);
...
function handleRes(){
    ...
    foreach($files as $file){
        ...
        if(@$pathinfo['extension'] == 'json'){
            $contents = file_get_contents(CLI_DATA_DIR . $file);
            if(json_decode($contents)){
                $file_size = filesize(CLI_DATA_DIR . $file);
                $file_obj = array('name' => $pathinfo['filename'], 'size' => $file_size, 'data' => $contents);
