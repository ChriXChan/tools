...
<?php
    ...
    function flat_normal_list($list) {
        ...
        for ($i = 0; $i < sizeof($list); $i++) {
            ...
            $num = $row['num'];
            $notice = $row['notice'];
            $msg = "#drop_prob_normal{npc_id = ". $npc_id. ", item_id = ". $item_id. ", domain = ". $domain. ", bind = ". $bind. ", num  = ". $num.", notice = ". $notice."}";
            if ($i == 0) {
                $rtl = $rtl.$msg;
            } else {
                $rtl = $rtl."\n, ".$msg;
    ...
    function flat_super_list($list) {
        ...
        for ($i = 0; $i < sizeof($list); $i++) {
            ...
            $num = $row['num'];
            $notice = $row['notice'];
            $msg = "#drop_prob_super{npc_id = ". $npc_id. ", item_id = ". $item_id. ", domain = ". $domain. ", upper_limit = ". $upper_limit. ", bind = ". $bind. ", num = ". $num.", notice = ". $notice. "}";
            if ($i == 0) {
                $rtl = $rtl.$msg;
            } else {
                $rtl = $rtl."\n, ".$msg;
        ...
        if($input != ""){
            ...
            for($i = 0; $i < count($arr_attr); $i++){
                if((trim($arr_attr[$i]))!=""){
                    ...
                    $output .= ",#drop_prob_super{npc_id = ".$npcid.", item_id = ".trim($arr_cond[0])
                        .", bind = ".trim($arr_cond[1]).", num = ".trim($arr_cond[2])
                        .", notice = ".trim($arr_cond[3])."}";               
                }
            }
        }
...
$temp_arr = array();
for ($i = 0; $i < count($data); $i++) {
    if(isset($temp_arr[$data[$i]['id']]))
        $temp_str = $temp_arr[$data[$i]['id']];
    else
        $temp_str = '';
    $super_str = get_cumulate_super($data[$i]['goods'], $data[$i]['id']);
    $drop_str = ",#drop_cumulate_base{count = {$data[$i]['count']}, goods = [$super_str]}\n    ";
    $temp_str .= $drop_str;
    $temp_arr[$data[$i]['id']] = $temp_str;
}
foreach($temp_arr as $k => $v){
?>
get_cumulate(<?php echo $k?>) -> 
    [<?php echo substr($v, 1)?>];
<?php }?>
...
for ($i = 0; $i < count($data); $i++) {
    $monid = $data[$i]['id'];
    $count = $data[$i]['count'];
    if (array_key_exists($monid,$temp_arr)) {
        if($temp_arr[$monid] <= $count ){
            $temp_arr[$monid] = $count;
        }
    }else{
        $temp_arr[$monid] = $count;
    }
